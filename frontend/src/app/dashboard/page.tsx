...
// Function to load all dashboard stats
const loadDashboardStats = async (): Promise<DashboardStats> => {
  try {
    // Load instances
    const instancesData = await fetcher('/api/whatsapp/instances');

    // Load numbers
    const numbersData = await fetcher('/api/user-numbers');
    const numbersList = Array.isArray(numbersData) ? numbersData : numbersData.numbers || [];

    // Load user config
    const configData = await fetcher('/api/config-user');
    ...
    return {
      instances: { count: 0, limit: 2 },
      numbers: { count: 0, limit: 4 },
...
export default function Dashboard() {
  ...
  // Use SWR for dashboard stats with caching
  // dedupingInterval: 60000ms (1 minute) - prevents duplicate requests within this time
  ...
  const { data: stats = {
    ...
    numbers: { count: 0, limit: 4 },
    userConfigured: false,
  }, isLoading: loading } = useSWR<DashboardStats>(
    'dashboard-stats',
    loadDashboardStats,
    {
      ...
      <div className="bg-white/95 backdrop-blur-sm rounded-2xl shadow-2xl p-6 border border-gray-200 mb-8">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
          <Link
            ...
            className="flex flex-col items-center justify-center p-6 bg-[#1A1A1A] hover:bg-[#252525] text-white rounded-xl transition-all transform hover:scale-[1.02]"
          >
      ...
      <div className="bg-white/95 backdrop-blur-sm rounded-2xl shadow-2xl p-6 border border-gray-200">
        ...
        <div className="flex flex-col sm:flex-row gap-4">
          <Link
            ...
            className="flex-1 text-center bg-[#1ECC5D] hover:bg-[#1AB84E] text-white font-semibold py-3 px-6 rounded-xl transition-all transform hover:scale-[1.02]"
          >
            Alterar Senha
          </Link>
          <button
            ...
            className="flex-1 text-center bg-[#EF4343] hover:bg-[#DC2626] text-white font-semibold py-3 px-6 rounded-xl transition-all transform hover:scale-[1.02] cursor-pointer"
          >
